...
class FilterRangeFactory extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (this.props.aditionalType !== nextProps.aditionalType) {
            const uiMin = this.transformFromDBValue(nextProps,
                Math.max(this.state.dbMinVal, this.state.dbMin), true
            );
            const uiMax = this.transformFromDBValue(nextProps,
        ...
        return (
            <LoadingWrapper loading={this.state.loading}>
                <ReactSlider
                    ...
                    withBars
                >
                    <div>{this.transformToUIName(this.props, this.state.uiMin)}</div>
                    <div>{this.transformToUIName(this.props, this.state.uiMax)}</div>
                </ReactSlider>
            </LoadingWrapper>
        );
    }
...
export class FilterAperture extends React.Component {
    ...
    transformFromDBValue = (props, value, isFirst) => {
        ...
        return _.findIndex(this.types[this.state.type], (v) => value < v) - (isFirst ? 1 : 0);
    }
...
FilterRating.transformToUIName = (props, value) => _.find(props.options, { value }).name;
