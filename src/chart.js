...
class Chart extends Component {
    ...
    render() {
        ...
        return (
            <LoadingWrapper loading={this.state.loading} noData={!this.state.data.columns.length}>
                {chartElement}
            </LoadingWrapper>
        );
    }
...
class BarChartComponent extends Component {
    ...
    expandDataset(rawData) {
        ...
        data.datasets.push({
            ...
            backgroundColor: rawData.values.map((v, i) => `hsla(${i * (360 / rawData.values.length)},90%,50%,0.5)`),
            hoverBackgroundColor: rawData.values.map((v, i) => `hsla(${i * (360 / rawData.values.length)},90%,50%,0.75)`),
            borderColor: rawData.values.map((v, i) => `hsla(${i * (360 / rawData.values.length)},90%,50%,0.8)`),
            hoverBorderColor: rawData.values.map((v, i) => `hsla(${i * (360 / rawData.values.length)},90%,50%,1.0)`)
        });
        return data;
    }
